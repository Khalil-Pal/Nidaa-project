...
`js/nidaa-common.js` is included once on every page that talks to the API, before
the page's own script. It defines the globals the page scripts use: `API` (relative
`/api`), `authHeader()`, `apiFetch()`, `logout()` (which also revokes the refresh
token server-side), `escHtml()`, `timeAgo()`, `currentUser()` and
...
or nav spacing. The sidebar markup itself is generated by `buildSidebar()` from one
role-to-pages table in the shared module; pages only provide an empty
`<nav id="sidebarNav">`.
