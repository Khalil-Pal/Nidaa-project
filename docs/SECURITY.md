...
| Identity | **Identity is revealed on assignment, everywhere.** Until a request is assigned, nobody outside its own parties learns who is behind it; on assignment the assigned provider or psychologist learns the beneficiary (and the beneficiary and the filer learn them), and nobody else ever does. The one sentence is the whole rule for contact details (`ContactInfoService`), for the "Filed on behalf of <name>" badge (`HelpRequestService.withBeneficiaryNames()`: the name only for the beneficiary, the filer, the assigned provider and an admin; a browsing provider sees that it was filed, not for whom) and for anonymous psychological requests, where the beneficiary withheld it and so nothing is revealed even on assignment: the psychologist supports the person, records sessions and receives ratings without ever seeing a name. | `ContactInfoService`, `HelpRequestService.withBeneficiaryNames()` (ON-2), `ConsultationService` (CS-1) |
| Consultation records | One row per session (V22); the assigned psychologist records on an ASSIGNED or COMPLETED case, the beneficiary rates each session once. `notes_for_psychologist` is returned only to the assigned psychologist; every other response, the administrator's included, is built without it and the key is absent from the JSON. No consultation response or notification carries the beneficiary's id or name (the identity rule above). Non-parties get **404**, for the list and for a session id that is not the case's. | `ConsultationService` (CS-1) |
...
| Declining an assignment | Only the request's own assigned provider may decline it, and only while it is `ASSIGNED`; every other caller gets **404**, so a provider cannot probe which requests exist. Declining never cancels the beneficiary's request: it returns to the queue, restores the reserved capacity exactly once (guarded UPDATE, ADR 005) and excludes the decliner from the rematch. Three declines escalate to administrators rather than looping. | `RequestDeclineService` (GAP-1) |
...
| Who learns whose identity follows one rule, stated once in 3.2 ("Identity is revealed on assignment, everywhere"): the transient `beneficiaryName` on a filed request is one of its three applications | `HelpRequestService.withBeneficiaryNames()` (ON-2) |
