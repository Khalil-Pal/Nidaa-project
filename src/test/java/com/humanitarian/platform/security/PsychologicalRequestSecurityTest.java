...
class PsychologicalRequestSecurityTest extends SecuritySliceTest {
    ...
    void psychologistCannotCloseUnassignedCase() throws Exception {
        ...
        verify(psychologicalRequestRepository, never()).updateStatusCompleted(anyLong(), anyString(), any(), anyString());
    }
    ...
    void assignedPsychologistCanCloseOwnCase() throws Exception {
        ...
        psychologistProfile(PSYCHOLOGIST_USER_ID, PSYCHOLOGIST_PROFILE_ID);
        storedRequest(1L, "ASSIGNED", PSYCHOLOGIST_PROFILE_ID);
        when(psychologicalRequestRepository.updateStatusCompleted(eq(1L), eq("COMPLETED"), any(), eq("ASSIGNED"))).thenReturn(1);

        mockMvc.perform(put("/api/psychological-requests/1/status").param("status", "COMPLETED"))
                .andExpect(status().isOk());
        // completion goes through the query that stamps completed_at (CS-1), guarded by the validated status (B-4)
        verify(psychologicalRequestRepository).updateStatusCompleted(eq(1L), eq("COMPLETED"), any(), eq("ASSIGNED"));
        verify(psychologicalRequestRepository, never()).updateStatusNative(anyLong(), anyString(), anyString());
    }
